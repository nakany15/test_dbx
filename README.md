...
```bash
conda create -n test_dbx python=3.9
conda activate test_dbx
```
...
```
This command will create your personal repository under `/Repos/<username>/test_dbx`.
3. To set up the CI/CD pipeline with the notebook, create a separate `Staging` repo:
```bash
databricks repos create --url <your repo URL> --provider <your-provider> --path /Repos/Staging/test_dbx
```
...
- To trigger the release pipeline, get the current version from the `test_dbx/__init__.py` file and tag the current code version:
```
